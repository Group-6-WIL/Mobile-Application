...
class Admin_Dashboard : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        contactBtn.setOnClickListener { addMember() }  // Calls method to add contact info
    ...
    private fun addMember() {
        ...
        val description = dialog.findViewById<EditText>(R.id.editTextEmail)
        val memberImageView = dialog.findViewById<ImageView>(R.id.adminC_image)
        ...
        save.setOnClickListener {
            ...
            val memberDescription = description.text.toString()

            if (memberName.isNotEmpty() && memberDescription.isNotEmpty()) {
                if (selectedImageUri != null) {
                    uploadImageAndSaveMember(memberName, memberDescription)
                } else {
                    saveMemberToDatabase(memberName, memberDescription, null)
                }
            } else {
    ...
    private fun uploadImageAndSaveMember(name: String, description: String) {
        val storageRef = FirebaseStorage.getInstance().reference.child("member_images/${UUID.randomUUID()}.jpg")
        selectedImageUri?.let { uri ->
            storageRef.putFile(uri)
                .addOnSuccessListener { taskSnapshot ->
                    storageRef.downloadUrl.addOnSuccessListener { uri ->
                        saveMemberToDatabase(name, description, uri.toString())
                    }
                }
    ...
    private fun saveMemberToDatabase(name: String, description: String, imageUrl: String?) {
        val memberId = FirebaseDatabase.getInstance().getReference("meet_the_member").push().key
        val member = Member(memberId, name, description, imageUrl)

        if (memberId != null) {
            FirebaseDatabase.getInstance().getReference("meet_the_member").child(memberId).setValue(member)
                .addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        Toast.makeText(this, "Member saved successfully!", Toast.LENGTH_SHORT).show()
                        dialog.dismiss()
                    } else {
                        Toast.makeText(this, "Failed to save member: ${task.exception?.message}", Toast.LENGTH_SHORT).show()
                    }
                }
        }
    }

    // Data class for Member with image URL
    data class Member(
        val Id: String?,
        val Name: String,
